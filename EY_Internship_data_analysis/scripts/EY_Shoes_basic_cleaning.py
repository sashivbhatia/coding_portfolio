...
# Load files
shoes_fact = pd.read_csv("C:\EY_Data\shoes_fact.csv")
shoes_dim = pd.read_csv("C:\EY_Data\shoes_dim.csv")
country_dim = pd.read_csv("C:\EY_Data\country_dim.csv")
forex_df = pd.read_csv("C:/EY_Data/daily_forex_rates.csv")

# Merge files by Country Code and ID
shoes_merged = shoes_fact.merge(country_dim[['country_code', 'currency', 'shoe_metric']], on='country_code', how='left')
shoes_merged = shoes_merged.merge(shoes_dim[['id', 'gender']], left_on='id', right_on='id', how='left')
...
def standardize_size(row):
    ...
    # Clean things like "10 1/2" -> "10"
    if ' ' in size:
        size = size.split()[0]

    # Fix for "10.5K" format in UK kids
    if metric == 'uk' and gender == 'K' and size.endswith('K'):
        size = size[:-1]  # ← fix: strip trailing K

    # Use 'U' for unknown/missing gender
...
shoes_merged.to_csv("C:/EY_Data/final_shoes_merged.csv", index=False)
